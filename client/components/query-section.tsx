...
interface QuerySectionProps {
  ...
  setQueryResults: (queryResults: { rows: any[][], colsNames: string[] }) => void;
...
export function QuerySection({ tableName, tableData, setSqlQuery, setQueryResults, sampleQueries }: QuerySectionProps) {
  const [query, setQuery] = useState('')
  const [loading, setLoading] = useState(false)
  

  console.log("sampleQueries", sampleQueries);
  const handleSubmit = async () => {
    ...
    try{setLoading(true);
      ...
      if (e instanceof Error) {
        toast('Error: ' + "The SQL Query can't be processed.. Please try again.", { icon: '❌', style: { color: 'white', backgroundColor: 'black' } });
      } else {
          ...
          <ScrollArea className="h-[15rem] mt-2">
            <TableHeader className="bg-[#051421] sticky top-0">
              <TableRow className='w-full'>
                {tableData?.cols && tableData.cols.map((col, i) => (
